...
architecture rtl of exp_continuos is
 --signal
 ...
 begin
	...
	Xneg_comp0 <= x"0100" when Xneg(15 downto 12) = "0000" else x"7FFF";
	...
	REGX1 : Regn
	generic map (16)
  	port map (RST, CLK, '1', X1_in, X1);
	REGX2 : Regn
	generic map (16)
  	port map (RST, CLK, '1', X2_in, X2);
	REGX3 : Regn
	generic map (16)
  	port map (RST, CLK, '1', X3_in, X3);
	REGX4 : Regn
	generic map (16)
  	port map (RST, CLK, '1', X4_in, X4);
	REGX12 : Regn
	generic map (16)
